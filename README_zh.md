...
在 host 和所有的 member 集群中调整升级配置文件，文件路径 `ks-installer/scripts/ks-core-values.yaml`，其中 `upgrade` 部分为升级相关的配置项目，你需要在升级之前确认需要升级的组件和配置信息，通过 `upgrade.jobs.config.<name>.enabled` 控制是否需要升级该组件，以下为配置示例：
...
在升级集群之前，执行以下 `ks-installer/scripts/pre-check.sh` 脚本文件，检查集群状态及是否满足升级条件。
